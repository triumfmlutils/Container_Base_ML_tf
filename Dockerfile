...
RUN rm -rv /usr/local/bin/jupyter* && python3 -m pip install --user --upgrade pip && python3 -m pip install --upgrade --force-reinstall jupyterlab && python3 -m pip install numpy uproot awkward vector atlas-mpl-style pydot graphviz xgboost lightgbm seaborn h5py plotly line_profiler keras tensorflow-addons
